...
export async function registerLivreurRoutes(app: FastifyInstance) {
  ...
  app.patch("/api/v1/livreurs/:livreurId/status", async (request, reply) => {
    ...
    if (!livreur) return reply.code(404).send({ error: "LIVREUR_NOT_FOUND" });
    return { livreur };
  });
  ...
  app.post("/api/v1/livreur-debts", async (request, reply) => {
    ...
    if (!debt) return reply.code(404).send({ error: "LIVREUR_NOT_FOUND" });
